...
log_name = label_name + '_standard_tr'
...
# standard auto-augmentation for cifar
...
trainset = OurDataset((train_x, train_y), transform=transform_train)
valset = OurDataset((val_x, val_y), transform=transform_test)
testset = OurDataset((test_x,), transform=transform_test)
...
for epoch in range(args.epochs):
    ...
    with open(log_name + '.txt', 'w') as f:
        f.write('epoch %d:train acc = %.4f, val acc = %.4f\n' % (
            epoch, train_accs[-1], val_accs[-1]))

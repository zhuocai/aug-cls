...
def transform(inputs: torch.Tensor, tr_type: int) -> torch.Tensor:
    ...
    if tr_type == 1:
        ...
        # color_jitter + color_gray + resize_crop + rotation
        out = color_jitter(inputs)
        out = color_gray(out)
        out = resize_crop(out)
        return rotation(out)
        ...
        # color_jitter + color_gray + resize_crop + cut_perm
        out = color_jitter(inputs)
        out = color_gray(out)
        out = resize_crop(out)
        return cut_perm(out)
        ...
        # rotation + cut_perm
        return cut_perm(rotation(inputs))

    elif tr_type == 5:
        # color_jitter + color_gray + resize_crop + rotation + cut_perm
        out = color_jitter(inputs)
        out = color_gray(out)
        out = resize_crop(out)
        return cut_perm(rotation(out))
